...
MPOSystem::MPOSystem(std::string filename) : ElectionSystem(), filename(filename) {}


void MPOSystem::setFilename(const std::string& newFilename) {
    filename = newFilename;
}

bool sortByPartyMPO(const std::pair<Party, int>& a, const std::pair<Party, int>& b) {
  return a.second > b.second;              
}


bool sortByCandidateMPO(const std::pair<Candidate, std::pair<Party, int>>& a,
                     const std::pair<Candidate, std::pair<Party, int>>& b) {
  return a.second.second > b.second.second;
}


std::pair<bool, int> isThereATieMPO(const std::vector<std::pair<Party, int>>& remainders, int seatsRemaining) {
...
void MPOSystem::processCandidates() {
  std::string line;
  std::ifstream file;
  file.open(filename);

  if (!file.is_open()) {
    std::cout << "\nUnable to open file. (MPO system)" << std::endl;
  }
...
void MPOSystem::countVotes() {
  ...
  int votesCounted = 0;
  while (votesCounted < votes && std::getline(file, line)) { // doing the && check slows down the program
...
void MPOSystem::giveVotesToParty() {
  ...
  std::sort(candidates.begin(), candidates.end(), sortByCandidateMPO);
  std::sort(parties.begin(), parties.end(), sortByPartyMPO);
}



// In a MPO election, any voter can give ONE candidate a vote, and only ONE candidate. 
// MPO is purely based on popularity, so if there a 4 seats, and the top 4 candidates
// come from the same party, then that Party's Candidates will get the 4 seats.
void MPOSystem::allocateSeats() {
...
std::string MPOSystem::displayResults() {
  std::string result = "A MPO election was run to fill " + std::to_string(getSeats()) +" seats. "
  ...
  result += MPOCandidates(candidates, winners, parties, partySeats, getVotesTotal());

  return result;
}

std::string MPOSystem::auditResults() {
  std::string result = "MPO Election Audit\n";
